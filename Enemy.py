...
class Enemy_monster:
    def __init__(self, spawn_position,move_range):
        ...
        self.count = 0
        self.apperance = 'sprite/Enemy_monster.png'
    ...
    def move(self):
        ...
        self.count += 1
        if ((self.count % 50) == 0):
            self.move_direction = -1 #np.random.randint(-1,2)
    ...
    def bottomCheck(self):
        ...
        if not collision:
            ...
            if (self.position[0] < self.move_range[0]):
                ...
                self.position[2] = self.position[0] + 48
            else :
                self.position[2] = self.move_range[1]
                self.position[0] = self.position[2] - 48


    def overlap_bottom(self, ego_position, other_position): #ego: 몬스터, otehr: 플랫폼
        return ego_position[0] > other_position[0] - 20 and ego_position[2] < other_position[1] + 20
